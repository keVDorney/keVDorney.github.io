...
t, xr, yr = Efield(lambda1=7.90e-7, ellip=1)
t, xb, yb = Efield(lambda1=3.95e-7, ellip=-1)

# Make combined field.
X = (xr + xb) / np.max(xr+xb)
Y = (yr + yb) / np.max(yr+yb)

# Multiply t by 1e15 to give it some real dimensions that Blender can handle.
make_field(t*1e15, X, Y, name='Counter Rotating')
